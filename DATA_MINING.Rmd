...
kable_styling(latex_options=c("striped","bordered","hold_position"),full_width = F,font_size = 16)%>%
...
```{r}
set.seed(124)
k<-predict(rf,test_data[,-4],type='prob')
pl<-test_data
pl$Bike_Injur<-as.factor(pl$Bike_Injur)
pl$Bike_Injur<-as.numeric(pl$Bike_Injur)
pl$Bike_Injur[pl$Bike_Injur==2 ]<-0
pl$Bike_Injur[pl$Bike_Injur==1 ]<-1
predrf<-prediction(k[,1],pl$Bike_Injur)
rfauc<-performance(predrf,"auc")@y.values[[1]]

k<-predict(bag,test_data[,-4],type='prob')
pl<-test_data
pl$Bike_Injur<-as.factor(pl$Bike_Injur)
pl$Bike_Injur<-as.numeric(pl$Bike_Injur)
pl$Bike_Injur[pl$Bike_Injur==2 ]<-0
pl$Bike_Injur[pl$Bike_Injur==1 ]<-1
prebag<-prediction(k[,1],pl$Bike_Injur)
bagauc<-performance(prebag,"auc")@y.values[[1]]
```


```{r}
T1<-rbind(c(rftest$byClass[c(1,2,5)],rfauc),c(bagtest$byClass[c(1,2,5)],bagauc))
T1<-cbind(T1)
rownames(T1)<-c("Random Forest","Bagging")
colnames(T1)<-c( "Sensitivity" ,"Specificity", "Precision" ,"AUC")
...
$\textbf{Conclusion}:$ Le Random forest permet un taux de précision superieur à celui du Random Forest.De plus
...
Les mesures de performances de ce modèle sont les suivantes :
```{r}
k<-predict(finalrf,test_data[,-4],type='prob')
pl<-test_data
pl$Bike_Injur<-as.factor(pl$Bike_Injur)
pl$Bike_Injur<-as.numeric(pl$Bike_Injur)
pl$Bike_Injur[pl$Bike_Injur==2 ]<-0
pl$Bike_Injur[pl$Bike_Injur==1 ]<-1
predrfa<-prediction(k[,1],pl$Bike_Injur)
aucrfa<-performance(predrfa,"auc")@y.values[[1]]

Tf<-rbind(c(c4$byClass[c(1,2,5)],aucrfa))
Tf<-cbind(Tf)
rownames(Tf)<-"Random Forest automatisé"
colnames(Tf)<-c( "Sensitivity" ,"Specificity", "Precision" ,"AUC")
kable(Tf,align = "c",caption = "Mesures de performance") %>%
kable_styling(latex_options=c("striped","hold_position"),full_width = F,font_size = 16) 
...
$\textbf{Conclusion :}$ Les AUC étant très similaires, pour le choix du meilleur modèle Boosting on se base sur la Précision : c'est donc le Boosting pénalisé à 0.01 qui est choisi.
...
```{r}
rr<-cbind(sensitivity(bossting_pen01$table),specificity(bossting_pen01$table),precision(bossting_pen01$table),performance(pred3,"auc")@y.values[[1]])



L1<-cbind(sensitivity(m.confusiontest1),specificity(m.confusiontest1),precision(m.confusiontest1),perftest1@y.values[[1]])
rownames(L1)<-"Scoring"
...
comp<-rbind(Ta,T1,Tf,rr,Llm)
kable(comp,align = "c",caption = "Comparaison des modèles", escape = TRUE) %>%
kable_styling(latex_options=c("striped","hold_position"),full_width = F,font_size = 16)
```
...
FrandomFA=(2 *Tf[1] * Tf[3]) / (Tf[1] + Tf[3])
...
kable(tableau,align = "c",caption = "Comparaison des F-mesure des modèles", escape = TRUE) %>%
kable_styling(latex_options=c("striped","hold_position"),full_width = F,font_size = 16)
